...
export function buildAnalysisPrompt(input: AnalysisPromptInput): string {
  const { articles, searchPhrase, currentDate } = input;
  const articlesJson = JSON.stringify(articles, null, 2);

  return `Jesteś ekspertem analizy mediów i komunikacji politycznej w kontekście polskiego krajobrazu medialnego.
...
export function prepareArticlesForPrompt(
